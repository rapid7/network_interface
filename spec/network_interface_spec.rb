        ...
        if hash.has_key?(:ipv4)
          describe "ipv4" do
            ...
            it "should match the system interface of #{hash[:ipv4]}" do
              expect(hash[:ipv4]).to include NetworkInterface.addresses(interface)[NetworkInterface::AF_INET][0]["addr"]
            end
          end
        end
        if hash.has_key?(:ipv6)
          describe "ipv6" do
            ...
            it "should match the system interface of #{hash[:ipv6]}" do
              expect(hash[:ipv6]).to include NetworkInterface.addresses(interface)[NetworkInterface::AF_INET6][0]["addr"]
            end
          end
        end
        if hash.has_key?(:mac)
